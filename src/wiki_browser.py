...
def show_wiki(g, EX, SCHEMA):
    ...
    filter_type = st.selectbox("Filter by:", 
        ["All", "Brand", "Architecture", "Release Year", "Memory Size (MB)", "Memory Type", "Memory Bus (bits)"])
    ...
    with st.form("wiki_filter_form"):
        ...
        with col_f1:
            ...
            if filter_type == "Brand":
                ...
                q_years = "SELECT DISTINCT ?year WHERE { ?y a <http://example.org/gpu/releaseYear> ; <https://schema.org/name> ?name . }"
                years = [str(r.name) for r in g.query(q_years)]
                filter_value = st.selectbox("Select year:", sorted(years))

            
            elif filter_type == "VRAM":
                q_m_sizes = "SELECT DISTINCT ?name WHERE { ?ms a <http://example.org/gpu/memorySizeMB> ; <https://schema.org/name> ?ms . }"
                m_sizes = [str(r.ms) for r in g.query(q_m_sizes)]
                ...
                q_m_buses = "SELECT DISTINCT ?mb WHERE { ?gpu <http://example.org/gpu/memBus> ?mb . }"
                m_buses = [str(r.mb) for r in g.query(q_m_buses)]
    ...
    if submitted:
        ...
        elif filter_type == "Memory Size (MB)":
            filter_clause = f'FILTER(?memorySize = {filter_value})'
        elif filter_type == "Memory Type":
            filter_clause = f'FILTER(STR(?memoryType) = "{filter_value}")'
        elif filter_type == "Memory Bus (bits)":
            filter_clause = f'FILTER(?busWidth = {filter_value})'
